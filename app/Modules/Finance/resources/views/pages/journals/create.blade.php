    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">New Journal Entry</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('finance.journals.store') }}" method="POST" id="journalFormPage">
                    @csrf
                    <div class="row mb-3">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-3">
                             ...
                             <select name="type" class="form-select" required>
                                 <option value="journal" {{ $type == 'journal' ? 'selected' : '' }}>Journal Voucher</option>
                                 <option value="payment" {{ $type == 'payment' ? 'selected' : '' }}>Payment Voucher</option>
                                 <option value="receipt" {{ $type == 'receipt' ? 'selected' : '' }}>Receipt Voucher</option>
                                 <option value="contra" {{ $type == 'contra' ? 'selected' : '' }}>Contra Voucher</option>
                             </select>
                        </div>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mt-4 d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" name="post_journal" value="1" class="btn btn-success">Save & Post</button>
                    </div>
                </form>
            </div>
        </div>
...
<template id="entryRowTemplate">
    <tr class="entry-row">
        <td>
            <select name="entries[INDEX][account_id]" class="form-select" required>
        ...
        <td>
            <input type="number" step="0.01" name="entries[INDEX][debit]" class="form-control debit-input" value="0.00">
        </td>
        <td>
            <input type="number" step="0.01" name="entries[INDEX][credit]" class="form-control credit-input" value="0.00">
        </td>
        <td class="text-center">
            <button type="button" class="btn btn-sm btn-ghost-danger remove-row"><i class="ri-delete-bin-line"></i></button>
        </td>
    </tr>
</template>
@endsection
