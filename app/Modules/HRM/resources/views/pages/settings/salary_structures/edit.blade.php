...
<div class="row clearfix">
    <div class="col-lg-12">
        <div class="card bg-white">
            ...
            <div class="card-body">
                <form action="{{ route('hrm.settings.salary-structures.update', $salaryStructure->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" name="description" value="{{ $salaryStructure->description }}">
                        </div>
                        <div class="col-md-12">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="isActive" name="is_active" value="1" {{ $salaryStructure->is_active ? 'checked' : '' }}>
                    ...
                    <div class="table-responsive mb-4 rounded border">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                ...
                                @php
                                    $hasComponent = $structureComponents->has($earning->id);
                                    $pivot = $hasComponent ? $structureComponents[$earning->id]->pivot : null;
                                @endphp
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input class="form-check-input earning-check" type="checkbox" name="components[{{ $earning->id }}][enabled]" value="1" id="c_{{ $earning->id }}" {{ $hasComponent ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    <td>
                    ...
                    <div class="table-responsive mb-4 rounded border">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                ...
                                @php
                                    $hasComponent = $structureComponents->has($deduction->id);
                                    $pivot = $hasComponent ? $structureComponents[$deduction->id]->pivot : null;
                                @endphp
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input class="form-check-input deduction-check" type="checkbox" name="components[{{ $deduction->id }}][enabled]" value="1" id="c_{{ $deduction->id }}" {{ $hasComponent ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    <td>
