...
<div class="row clearfix">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('hrm.settings.leave-types.update', $leaveType->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" name="code" value="{{ $leaveType->code }}" required>
                        </div>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">Description</label>
                        <textarea class="form-control" name="description" rows="2">{{ $leaveType->description }}</textarea>
                    </div>
                    ...
                    <div class="row align-items-center mb-3">
                        <div class="col-md-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="is_unlimited" name="is_unlimited" onchange="toggleLimit(this)" {{ $leaveType->is_unlimited ? 'checked' : '' }}>
                        ...
                        <div class="col-md-4" id="days-container">
                            ...
                            <input type="number" class="form-control" name="days_per_year" value="{{ $leaveType->days_per_year }}" min="0" {{ $leaveType->is_unlimited ? 'readonly' : '' }}>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="is_paid" name="is_paid" {{ $leaveType->is_paid ? 'checked' : '' }}>
                                <label class="form-check-label" for="is_paid">Is Paid Leave?</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="requires_attachment" name="requires_attachment" {{ $leaveType->requires_attachment ? 'checked' : '' }}>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control" name="carry_forward_limit" value="{{ $leaveType->carry_forward_limit }}" min="0" placeholder="0 for none">
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="approval_chain_id">
                            <option value="">Standard Approval</option>
                            @foreach($approvalChains as $chain)
                            <option value="{{ $chain->id }}" {{ $leaveType->approval_chain_id == $chain->id ? 'selected' : '' }}>{{ $chain->name }} ({{ $chain->levels->count() }} Levels)</option>
                            @endforeach
                        </select>
                    </div>
